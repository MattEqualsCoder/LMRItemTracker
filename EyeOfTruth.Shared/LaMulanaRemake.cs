...
namespace LMRItemTracker.EyeOfTruth.Shared;
...
public class LaMulanaRemake : WatchedProcess
{
    ...
    DeepPointer bytesptr = new(0), wordsptr = new(0);
    ...
    public override bool Attach()
    {
        ...
        if (!Attach((Process p) =>
            {
                ...
                try
                {
                    return p.MainModule != null && p.MainModule.FileVersionInfo.ProductName == "La-Mulana";
                }
        ...
        if (version == null || !veroffsets.TryGetValue(version, out offsets))
            return false;
